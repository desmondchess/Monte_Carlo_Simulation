...
X = JLD2.load("crater_lake_coords.jld2")

# ╔═╡ f8dc6b48-f44b-4fbe-81ee-9872b7eadaf0


# ╔═╡ 61fd6a1c-f866-450f-a44b-8636c0520b84
X["X"]

# ╔═╡ 8a9dbb23-79d3-43e8-a07d-e8db2f11d865
begin
	x₁ = [value[:1,:] for (key,value) in X][1]
	println(x₁)
end

# ╔═╡ 5a95c728-dd8d-454c-aae0-da355fd741b2
begin
	x₂ = [value[:2,:] for (key,value) in X][1]
	println(x₂)
end
...
# ╔═╡ af4ebc34-a8c8-43a0-bd4f-554eb047df0e
begin
typeof(X["X"])
end

# ╔═╡ 3ef0f82f-e8e1-4c84-974c-ef1c0bb162d6
begin
	f = Figure()
	ax = Axis(f[1, 1],
    title = "Crater_lake_Scatterplot",
    xlabel = "In Miles",
    ylabel = "In Miles",
	aspect=DataAspect()
)
...
# ╔═╡ 130bf90c-3d6b-4401-b68f-0e43ad19bb2a
x̄ = mean(X["X"],dims=2)
...
# ╔═╡ 955d7f72-9ee0-4eb7-bf2d-b34b2fea7a7d
x̄1 = X["X"] .- x̄

# ╔═╡ 7f8ef199-c227-4187-aa26-9e45cdb49cf1
begin
	x₃ =x̄1[1,:]
	x₄ =x̄1[2,:]
end

# ╔═╡ 13e444b6-a1e0-4fc6-a60a-c6fca6eb718b
typeof(x₃)
...
begin
	...
	ax₁ = Axis(fi[1, 1],
    title = "Crater_lake_Scatterplot_2",
    xlabel = "In Miles",
    ylabel = "In Miles",
	aspect=DataAspect()
)
...
# ╔═╡ 282c7462-91fd-42cb-b0a4-dd1c9c0b4dcc
begin
	r = zeros(38)
...
# ╔═╡ c464afc2-7737-4131-8f34-881b754ab7c0
typeof(r)
...
# ╔═╡ 66b043b8-3f43-451d-8a4d-09fc0b2f7adc
begin
	θ = zeros(38)
	for (i , xᵢ) in enumerate(x₃)
		θ[i] = atan(x₄[i], xᵢ)
...
# ╔═╡ 7f66f42a-270c-4fe7-82c0-6dda04f3da55
θ
...
# ╔═╡ a0d84b86-ddaf-4edf-9413-6f426f04a9c3
begin
	xₚₒₗ = zeros(38)
	yₚₒₗ = zeros(38)
...
# ╔═╡ 1ad28c3f-6147-436b-812a-bd8f1ef1bad8
#xₚₒₗ
yₚₒₗ

# ╔═╡ ede2c69e-a826-4afd-b6b9-6a6361551021

begin
	fig = Figure()
	ax₂ = Axis(fig[1, 1],
    title = "Crater_lake__bound_Scatterplot_3",
    xlabel = "In Miles",
    ylabel = "In Miles",
	aspect=DataAspect()
)
...
# ╔═╡ b2a2d7b5-e387-4c5b-8211-521e8ce53b78
ind = sortperm(θ) 

# ╔═╡ 1d8652b4-9a46-4d24-b2d5-191337e8174d
begin
		r_sorted = r[ind]
		θ_sorted = θ[ind]
...
# ╔═╡ 9818b4f0-3e45-41bf-8383-1a29ed4452fd
interp_linear₁ = linear_interpolation(θ_sorted,r_sorted)
...
begin
	nx_sorted = prepend!( θ_sorted,([θ_sorted[end] - 2π]))
	ny_sorted = prepend!( r_sorted,[r_sorted[end]])
end

# ╔═╡ 3505b1b9-904b-462a-ae77-2758e0aa73f0
nx_sorted
...
r_of_θ = linear_interpolation(nx_sorted,ny_sorted, extrapolation_bc=Periodic())
...
# ╔═╡ dd67a034-2572-494f-80ee-0b9a0355f385
begin
	r_of_θ[1] 
	r_of_θ[1 + 2π]
end
...
# ╔═╡ 27bb3490-03c6-4878-bad0-b73a81585001
begin
	xx = zeros(500)
	yy = zeros(500)
	for (i,y) in enumerate(list)
...
# ╔═╡ e97a5072-5f73-4d9f-9229-90f4abb046bc
begin
	figu = Figure()
	ax₃ = Axis(figu[1, 1],
    title = "Crater_lake__bound_Scatterplot_4",
    xlabel = "In Miles",
    ylabel = "In Miles",
	aspect=DataAspect()
)
...
# ╔═╡ 8a4081ae-8d7c-4040-bf99-9a8a35d08ba5
r_of_θ[1]

# ╔═╡ 643006b0-d36e-4c7d-8179-5e52fa3366e1
function inside_lake(x, r_of_θ)
	x₁,y₁ = x 
	θs = atan(y₁/x₁)
	rs = sqrt(y₁^2 + x₁^2)
	func = r_of_θ[θs]
...
# ╔═╡ 234efc4c-4b70-4008-9f19-5a14e7aecc51
new_rθ
...
# ╔═╡ 6131eef1-5d85-4a11-b7a4-6ca87b8a53fc
...
function run_monte_carlo(N , L, r_of_θ = r_of_θ)
	dt = Uniform(-L/2, L/2)
	X = rand(dt, (2,N))
	xᵢ =X[1, :]
	yᵢ = X[2, :]
	...
	return (points=X, confirm=inside_lake)
end
...
begin
	N = 10000
	monte_carlo = run_monte_carlo(N, L, r_of_θ)
	
end

# ╔═╡ 09076209-2dba-4314-b138-f209dbd68d3f
area_of_lake = L^2 * (sum(monte_carlo.confirm)/ N)
...
# ╔═╡ c382ed0e-9e16-4ba7-8b44-125095ca1eed
list

# ╔═╡ ebc3ef91-e4cc-4384-9bbf-7f9ef90a9c8a
zip(new_rθ, list)

# ╔═╡ ef063d0c-cf6f-4fe4-9f30-890a7e54bed9
function draw_monte_carlo()
	figur =Figure()
	axᵢ = Axis(figur[1,1], xlabel = "x [miles]", ylabel = "y [miles]", 
		       aspect= DataAspect(), title = "Monte_Carlo_Crater_Lake ")
	x = [rᵢ * cos(θ) for (rᵢ, θ) in zip(new_rθ, list)]
	y = [rᵢ * sin(θ) for (rᵢ, θ) in zip(new_rθ, list)]
	lines!(x, y, color=:blue)

	xᵢ =monte_carlo.points[1, :]
	yᵢ = monte_carlo.points[2, :]
	
	for (i,val) in enumerate(xᵢ)
		θᵢᵢ = atan(yᵢ[i], val)
		rᵢᵢ = sqrt(yᵢ[i]^2 + val^2)
		rₛₒᵣₜ = r_of_θ(θᵢᵢ)
		if rᵢᵢ>rₛₒᵣₜ
			scatter!(axᵢ, val, yᵢ[i], marker=:+, color=:brown)
		else
			scatter!(axᵢ, val, yᵢ[i], color=:blue)
		end
	end
	figur
...
# ╔═╡ 899f7580-90da-488c-9db6-c9892241137b
md"
## Answers
"

# ╔═╡ f7fd689c-6554-418c-8704-c18e6a613224
md"
the large difference in variance (between both 100 and 10000) is made possible due to the value of N, which is our number of raindrops. it can be observed from the montecarloplot that when N=100, there are lots of random points that the raindrops could touch both inside and outside the lake, hence, this gives an insufficient method for calculating the actual area of the lake. while more larger values of N ensures more surface area is covered and a close to perfect deduction of the lake area"

# ╔═╡ 9d4b9ae3-4b6c-4b9e-964c-6eb63713071c
md"
In other to account for the water only (excluding the wizard island), entails collecting the data points (x and y axis) of wizard island and basically following the same steps like we did above when mapping out the area of the lake to map out the the area of wizard island. Then finally, subtracting the area of wizard island from the area of crater lake to ascertain the exact value of the crater lake"

# ╔═╡ 2aa24b28-b9a9-4944-b25d-dd07065f2aee
md"
Personally, i feel parameterization would work with all lakes, as long as we split the lakes into cross sections, work seperately on them then sum them up to get the required area. for example, using lake huron as a case study, i think an inside_lake function would entail FOR loop and IF statements to check if a point is in our various interpolated functions. Also, for cases like this, i think its important to note that each cross sections do not have new boundaries but each are part of the whole that covers the lake"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "e1c979624037d0afce4fc0ddea541e77569ab877"
